...
- AI 分析与向量写入 ArkData v3，本机完成余弦相似度匹配和语义关系生成；
...
本地数据库名为 `fragment_box.db`，使用 ArkData RDB。新碎片只有在用户明确选择“匿名加入社区”后才获得社区可见性；AI 处理也必须对每张碎片单独授权。撤回共享会把可见性改为 `REVOKED`，社区入口立即过滤该碎片。永久删除会清理本地关系、回响、共享授权、AI 分析、向量、AI 授权和社区收存索引。
